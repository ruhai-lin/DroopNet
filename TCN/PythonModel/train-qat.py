...
def main():
    print("-" * 60)
    print(f"TinyTCN QAT Training | Device: {DEVICE}")
    print("-" * 60)
    ...
    model = TinyTCNInt8()
    model.fuse_model()
    model.qconfig = tq.get_default_qat_qconfig(Q_ENGINE)
    tq.prepare_qat(model, inplace=True)
    model = model.to(DEVICE)
    
    print(f"Model Estimate (Float QAT): {model.footprint_bytes()} bytes")
    ...
    for epoch in range(EPOCHS):
        ...
        if epoch == FREEZE_OBSERVER_EPOCH:
            print(">> [QAT] Freezing Observers (Min/Max ranges locked)")
            model.apply(tq.disable_observer)
        
        if epoch == FREEZE_BN_EPOCH:
            print(">> [QAT] Freezing BN Statistics (Mean/Var locked)")
    ...
    # Save standard PyTorch QAT checkpoint (Float)
    torch.save(model.state_dict(), "../outputs/tiny_tcn_qat_float.pth")

    # 5. Conversion & Export
    print("-" * 60)
    print("Converting to INT8 CPU Model...")
